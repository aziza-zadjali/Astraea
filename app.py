def main():
    ...
    with st.sidebar:
        ...
        option = st.radio(option_text[lang_code], feature_options[lang_code], key="feature_select")
    ...
    if option == feature_options[lang_code][0]:
        document_query_feature(lang_code)
    elif option == feature_options[lang_code][1]:
    ...
    for item in st.session_state.chat_history:
        if isinstance(item, tuple) and len(item) == 2:
            ...
            st.warning(f"Invalid chat history item: {item}")
